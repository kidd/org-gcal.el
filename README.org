...
** Strip HTML from event descriptions

Google Calendar returns event descriptions as HTML. By default, org-gcal
preserves that HTML so event descriptions can be edited locally without losing
formatting.

If you prefer plain text descriptions in Org, set
=org-gcal-strip-html-descriptions= to =t=:

#+BEGIN_SRC emacs-lisp
  (setq org-gcal-strip-html-descriptions t)
#+END_SRC

You can override this global default for individual calendars with
=org-gcal-strip-html-descriptions-overrides=. This is useful when you want to
strip HTML for most calendars but preserve it for shared calendars:
